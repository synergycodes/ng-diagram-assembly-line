...
export const PROPERTIES: Record<NodeType, PropertyMeta[]> = {
  ...
  [NODE_TYPES.BUFFER]: [
    ...
    {
      // Thresholds act on the fill percentage, not the raw count — see
      // `deriveBufferLevel`, which feeds `current/capacity × 100` into `computeTone`.
      key: 'currentCount',
      label: 'Current',
      numeric: true,
      min: 0,
  ...
  [NODE_TYPES.WELDING_CELL]: [
    ...
    { key: 'totalRobots', label: 'Total robots', numeric: true, min: 0, max: 4 },
