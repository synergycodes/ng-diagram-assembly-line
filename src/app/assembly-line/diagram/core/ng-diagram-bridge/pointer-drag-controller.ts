...
export class PointerDragController<TState> {
  ...
  teardown(): void {
    if (!this.state && this.listenerEl === null) {
      return;
    }
    this.cleanup();
    this.state = null;
  ...
  private readonly onPointerMove = (event: PointerEvent): void => {
    if (!this.state || event.pointerId !== this.pointerId) {
      return;
    }
    ...
    this.rafHandle = requestAnimationFrame(() => {
      ...
      if (pending && this.state) {
        this.handlers.onMove(pending, this.state);
      }
    });
  };
  ...
  private readonly onPointerUp = (event: PointerEvent): void => {
    ...
    if (!state || event.pointerId !== this.pointerId) {
      return;
    }
    this.cleanup();
    this.state = null;
