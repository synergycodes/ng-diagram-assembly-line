// Reshapes an orthogonal edge when its endpoints move (a node drag, or an
// endpoint re-anchored to its port), keeping every segment axis-aligned and
// preserving the user's interior bends — the bend-preserving counterpart to a
// naive endpoint replace.
...
export function stretchPolylineWithBendInsertion(
  ...
  if (strict) {
    return collapseCollinearBends(strict);
  }
...
// 3-point routes left by re-anchoring endpoints to misaligned ports. L corners stay.
