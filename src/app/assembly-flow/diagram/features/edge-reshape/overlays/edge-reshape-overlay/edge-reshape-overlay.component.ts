...
export class EdgeReshapeOverlayComponent {
  ...
  private maskInjectedBends(
    ...
    for (const segment of segments) {
      if (sourceBendInserted && segment.segmentIndex === 0) {
        continue;
      }
      if (targetBendInserted && segment.segmentIndex === targetBendLiveIndex) {
        continue;
      }
  ...
  private classifyEndpoint(nodeId: string): ReshapeEndpointKind {
    if (this.extension) {
      return this.extension.classifyEndpoint(nodeId);
    }
...
function neighborAxis(
  ...
  const isSameX = sameX(end, neighbor);
  const isSameY = sameY(end, neighbor);
  if (isSameX && !isSameY) {
    return 'vertical';
  }
  if (isSameY && !isSameX) {
...
function orthogonalizePolyline(
  ...
  const result: { x: number; y: number }[] = [clonePoint(points[0])];
  for (let i = 1; i < points.length; i++) {
    ...
    // Vertical-first: bend down/up first, then run horizontally to the target.
